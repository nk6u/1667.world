...
$(document).ready(() => {
  const links = [
    {
      name: 'Telegram',
      link: 'https://t.me/wickedempire',
    },
    {
      name: 'Playlist',
      link: 'https://open.spotify.com/playlist/5TnIJU1ziw1A8o2Lps3iS9?si=6b63ad55a7124fd6',
    },
    {
      name: 'Discord',
      link: 'https://discord.com/users/864468880654204969/',
    },
    {
      name: 'Instagram',
      link: 'https://www.instagram.com/anymics/',
    },
    {
      name: 'tiktok',
      link: 'https://www.tiktok.com/@fyca',
    },
  ];
  ...
  for (let i in links) {
    ...
    if (i != links.length - 1) $('#marquee').append(' <img class="emoticon" src="assets/others/> ');
  }
  ...
  app.titleChanger(['lyk#0001', '@fyca', 'feds.lol/prop ', '@anymics', '.gg/paralyzed', 'jalen', '##wicked', '##rich']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #EE4B2B'>created by lyk#0001</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
